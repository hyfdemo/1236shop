...
class weixinapi{
	...
	function followUser($wxid,$info=array()){
		...
		if($id>0){
			...
			$sql = "update {$this->_pre}user set isfollow=1{$set} where uid={$id}";
		}else{
			$createtime = time();
			$createymd = date('Y-m-d');
			$sql = "insert into {$this->_pre}user (`ecuid`,`fake_id`,`createtime`,`createymd`,`isfollow`,`nickname`,`sex`,`country`,`province`,`city`,`access_token`,`expire_in`,`headimgurl`) 
				value (0,'{$wxid}','{$createtime}','{$createymd}',1,'{$nickname}','{$sex}','{$country}','{$province}','{$city}','{$access_token}','{$expire_in}','{$headimgurl}')";
		}
		$GLOBALS['db']->query($sql);
		return true;
	}
	...
	**/
	...
	function doAward($aid){
		...
		$GLOBALS['db']->query("INSERT INTO `weixin_actlog` (uid,class_name,createymd,createtime,code,issend) 
		value ($uid,'$class_name','$ymd','$time','$code',0)");
		$class_name = $class_name ? "恭喜你中奖了！<br>奖品:".$class_name : "非常遗憾没有中奖！";
		return array('num'=>$awardNum,'msg'=>$msg,'prize'=>$class_name,'url'=>'/mobile/user.php?act=address_list');
	}
	function randAward($aid){
		if(intval(rand(1,5)) != 1) return false;
		$actList = $GLOBALS['db']->getAll ( "SELECT lid,randnum,awardname,num FROM `weixin_actlist` where aid=$aid and isopen=1 and num>num2 order by num desc" );
		if($actList){
